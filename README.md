...
![image](https://user-images.githubusercontent.com/36217195/118239248-7988b180-b4d4-11eb-8565-43438a1d50c9.png)
...
공유 자전거 서비스 - [TODO] https://1sung.tistory.com/106

공유자전거(gBike) 시나리오
1. 사용자가 앱/웹을 통해 회원가입을 한다.
2. 사용자가 자전거를 검색한다. (위치, 가용한 자전거 수 등...)
  - Offline 이동 및 자전거 선택 - 
3. 사용자가 자전거의 QR코드로 사용 요청 한다. (체크인 - 로그인 되어 있지 않으면 반드시 로그인 시킨다.)
3-1. 자전거가 사용불가 상태 일 경우 사용자에게 사용 불가 안내하고, 다른 자전거를 제안한다. 
4. 사용자는 자전거 사용 완료 후 체크아웃 요청 한다. (결재할 금액을 보여줌)
5. 체크아웃 요청이 오면 시스템에서 결제처리 한다. 
8. 결제를 완료해야 자전거 이용이 종료된다.  - 결제 이전에는 계속 서비스 이용중  (자전거위치 및 배터리 잔량을 저장한다. )

비기능적 요구사항
  1. 트랜잭션
     - QR코드 스캔 시 자전거 사용가능 상태를 받아 사용가능 상태 일 때만 이용 가능 하도록 해야 한다 (sync 호출)
  2. 장애격리
     - 결제 기능이 정상이 아니더라도 체크아웃은 항상 수행되어야 하며 기능 정상화 후 추후 결제처리 될 수 있도록 한다.
       (Async, event-driven, Eventual consistency)
     - 체크인 시스템이 과중되면 체크인 호출을 잠시 막고 잠시 후에 하도록 유도한다. (circuit breaker, fallback)
  3. 성능
     - 사용자가 자전거 위치를 항상 확인 할 수 있는 화면을 제공해야 한다. (CQRS)
  4. 운영
